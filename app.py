...
try:
    test_redis = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, password=REDIS_PASSWORD, ssl=REDIS_SSL, decode_responses=True)
    test_redis.ping()
    ...
    limiter = Limiter(get_remote_address, app=app, default_limits=["200 per day", "50 per hour"])
    logger.warning(f"Redis unavailable for rate limiting: {e}. Falling back to in-memory limiter.")
...
def index():
    return jsonify({
        ...
        "version": "4.1",
        "endpoints": ["/health", "/dashboard", "/webhook/voice", "/webhook/dtmf", "/inbound-sms", TELEGRAM_WEBHOOK_PATH],
        "telegram_webhook": {
            "path": TELEGRAM_WEBHOOK_PATH,
            "webhook_url": TELEGRAM_WEBHOOK_URL or None,
            "secret_token_configured": bool(TELEGRAM_WEBHOOK_SECRET_TOKEN),
...
def telegram_webhook():
    ...
    try:
        ...
        logger.exception(f"Telegram webhook process failed: {e}")
